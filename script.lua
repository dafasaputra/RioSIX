...
-- Instant Fishing (RioSIX.lua Pattern - No Delay)
...
local function RefreshConfigList()
    ...
    local success, files = pcall(function() return listfiles("RioSIX_Configs") end)
...
local function LoadConfig(configName)
    ...
    local success, content = pcall(function() return readfile("RioSIX_Configs/" .. configName .. ".json") end)
    if not success then 
...
SaveBtn.MouseButton1Click:Connect(function()
    ...
    if not jsonSuccess then
        ...
        warn("RioSIX SAVE ERROR (JSON):", encodedData)
        return
    end

    local success, err = pcall(function()
        if not isfolder("RioSIX_Configs") then makefolder("RioSIX_Configs") end
        writefile("RioSIX_Configs/" .. name .. ".json", encodedData)
    end)
    ...
    if success then
        ...
        warn("RioSIX SAVE ERROR (WRITE):", err)
    end
end)
...
DeleteBtn.MouseButton1Click:Connect(function()
    ...
    delfile("RioSIX_Configs/" .. selectedConfig .. ".json")
...
local function TestWebhook(url, name)
    ...
    task.spawn(function()
        local p = { content = "✅ **TEST:** " .. name .. " Connected!", username = "RioSIX", avatar_url = "https://i.imgur.com/CWWGnhO.jpeg" }
...
local function CheckAndSendNonPS(isManual)
    ...
    task.spawn(function()
        local p = { ["username"] = "RioSIX", ["avatar_url"] = "https://i.imgur.com/CWWGnhO.jpeg", ["content"] = contentMsg, ["embeds"] = {{ ["title"] = "Player Not On Server", ["description"] = "```\n" .. txt .. "\n```", ["color"] = 16733440, ["footer"] = { ["text"] = "RioSIX Webhook", ["icon_url"] = "https://i.imgur.com/CWWGnhO.jpeg" } }} }
...
BtnPS.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local p = { ["username"] = "RioSIX", ["avatar_url"] = "https://i.imgur.com/CWWGnhO.jpeg", ["embeds"] = {{ ["title"] = " Manual Player List", ["description"] = "```\n" .. str .. "\n```", ["color"] = 5763719, ["footer"] = { ["text"] = "RioSIX Webhook", ["icon_url"] = "https://i.imgur.com/CWWGnhO.jpeg" } }} }
        httpRequest({ Url = Current_Webhook_List, Method = "POST", Headers = {["Content-Type"]="application/json"}, Body = HttpService:JSONEncode(p) })
    end)
end)
                     ...
                     task.spawn(function()
                         ...
                         local embed = {
                             ...
                             ["embeds"] = {{
                                 ...
                                 ["color"] = 16776960,
                                 ["footer"] = { ["text"] = "RioSIX Webhook", ["icon_url"] = "https://i.imgur.com/CWWGnhO.jpeg" }
                             }}
                         }
...
local function SendWebhook(data, category)
    ...
    local embedTable = { ["title"] = embedTitle, ["description"] = descriptionText, ["color"] = embedColor, ["footer"] = { ["text"] = "RioSIX Webhook", ["icon_url"] = "https://i.imgur.com/CWWGnhO.jpeg" } }

    if category == "SECRET" then
        embedTable["thumbnail"] = { ["url"] = "https://i.imgur.com/CWWGnhO.jpeg" }
        embedTable["footer"] = { ["text"] = "RioSIX Webhook" }
    end

    local embedData = { ["username"] = "RioSIX", ["avatar_url"] = "https://i.imgur.com/CWWGnhO.jpeg", ["content"] = contentMsg, ["embeds"] = {embedTable} }
...
local function FastInfiniteRejoin()
    if not ScriptActive then return end
    print("🔄 RioSIX: Mencoba reconnect setiap 5 detik...")
    while ScriptActive do
        ...
        if success then print("✅ RioSIX: Perintah reconnect berhasil dikirim!") break else warn("⚠️ RioSIX: Gagal, mencoba lagi dalam 5 detik...") end
        task.wait(5)
    end
end
...
local function SendDisconnectWebhook(reason)
    ...
    if tick() - LastDisconnectTime < 30 then 
        print("⚠️ RioSIX: Disconnect Webhook Cooldown Active")
        return 
    end
    LastDisconnectTime = tick()
    
    print("⚠️ RioSIX: Sending Disconnect Webhook (Reason: " .. tostring(reason) .. ")")
...
print("✅ RioSIX System Session v1.0 Loaded!")
